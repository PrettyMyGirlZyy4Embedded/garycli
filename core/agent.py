...
_MICROPYTHON_SOFT_RESET_TOOLS = {"rp2040_soft_reset", "esp_soft_reset", "canmv_soft_reset"}
...
def stm32_connect(chip: str = None) -> dict:
    ...
    ctx = get_context()
    if is_generic_micropython_name(chip):
        ctx.chip = "MICROPYTHON"
        return _esp_connect_impl(
            "MICROPYTHON",
            port=None,
            baud=SERIAL_BAUD,
...
class STM32Agent:
    def __init__(self, interactive: bool = True):
        ...
        self._skill_prompt_signature: tuple[Any, ...] | None = None
        ...
        self._turn_tool_attempts: dict[tuple[str, str], list[dict[str, Any]]] = {}
        self._turn_micropython_recovery_state: dict[str, Any] = {}
    ...
    def _compose_system_prompt(self) -> str:
        ...
        skill_prompt = self._current_skill_prompt()
        ...
        system_signature = (
            self._static_prompt_signature,
            self._skill_prompt_signature,
            self._dynamic_prompt_signature,
        )
    ...
    def _remove_web_research_hints(self):
        """Drop legacy transient system hints from conversation history."""
    ...
    def _prepare_web_research_hint(self, user_input: str):
        """Prepare a one-turn research directive without adding extra system roles."""
        ...
        if not should_force_web_research(user_input):
            return
        ctx = get_context()
        self._pending_system_hint = build_web_research_hint(ctx.cli_language)
    ...
    def _current_skill_prompt(self) -> str:
        """Return prompt additions only for skills relevant to the current tool subset."""

        if not hasattr(self, "_skill_prompt_cache") or self._skill_prompt_cache is None:
            self._skill_prompt_cache = ""
        if not hasattr(self, "_skill_prompt_signature"):
            self._skill_prompt_signature = None
        if not hasattr(self, "_tool_schema_cache") or self._tool_schema_cache is None:
            self._tool_schema_cache = []
        ...
        get_prompt_additions = getattr(manager, "get_prompt_additions_for_tools", None)
        skill_prompt = ""
        if callable(get_prompt_additions):
            try:
                skill_prompt = str(get_prompt_additions(set(active_tool_names)) or "")
            except Exception:
                skill_prompt = ""
        signature = (active_tool_names, skill_prompt)
        if signature != self._skill_prompt_signature:
            self._skill_prompt_cache = skill_prompt
            self._skill_prompt_signature = signature
    ...
    def _context_usage(self) -> dict[str, int]:
        ...
        request_tokens = int(estimate.get("total_tokens") or 0)
        base_tokens = self._base_context_tokens()
        used_tokens = max(0, request_tokens - base_tokens)
        limit_tokens = max(1, MAX_CONTEXT_TOKENS - base_tokens)
        left_tokens = max(0, limit_tokens - used_tokens)
        left_percent = 0
        if limit_tokens > 0:
            left_percent = max(
                0,
                min(100, int(round((left_tokens / limit_tokens) * 100))),
            )
        return {
    ...
    def _truncate_history(self):
        ...
        total = usage["used_tokens"]
        limit = usage["limit_tokens"]
        while total >= limit and len(self.messages) > 3:
    ...
    def _messages_for_api(self) -> list:
        ...
        pending_hint = str(getattr(self, "_pending_system_hint", "") or "").strip()
        ...
        if pending_hint:
            system_parts.append(pending_hint)
    ...
    def _reset_turn_tool_state(self) -> None:
        """Clear per-turn tool retry guards."""

        self._turn_tool_attempts = {}
        self._turn_micropython_recovery_state = {
            "soft_reset_success": False,
            "post_reset_deploy_failed": False,
        }

    def _tool_signature(self, name: str, args: dict[str, Any]) -> tuple[str, str]:
        """Build a stable per-turn signature for one tool call."""

        try:
            normalized = json.dumps(args or {}, ensure_ascii=False, sort_keys=True)
        except Exception:
    ...
    def _should_block_redundant_tool_call(self, name: str, args: dict[str, Any]) -> dict[str, Any] | None:
        ...
        if previous_failures:
            ...
            return {
                ...
                "blocked_retry": True,
                "message": (
                    f"当前回合里 `{name}` 已用相同参数失败过一次。"
                    "不要继续重复同一个 MicroPython 恢复工具；请基于已有错误直接总结下一步。"
                ),
        ...
        if recovery_state.get("soft_reset_success") and recovery_state.get("post_reset_deploy_failed"):
            return {
                ...
                "message": (
                    "当前回合已执行过一次 MicroPython soft reset，且复位后的 flash/auto_sync 仍失败。"
                    "不要再继续反复 connect/list_files/flash/auto_sync；请直接告诉用户需要手动按 RST、重插 USB，"
                    "或先修正板端脚本后再试。"
                ),
            }
        return None

    def _record_turn_tool_attempt(self, name: str, args: dict[str, Any], result_obj: Any) -> None:
        """Track per-turn tool outcomes for retry suppression."""

        if not isinstance(result_obj, dict):
            return
        signature = self._tool_signature(name, args)
        self._turn_tool_attempts.setdefault(signature, []).append(
            {
                "success": bool(result_obj.get("success", False)),
                "message": str(result_obj.get("message") or result_obj.get("error") or ""),
                "raw_repl_failure": bool(result_obj.get("raw_repl_failure")),
            }
        )
        if name in _MICROPYTHON_SOFT_RESET_TOOLS and result_obj.get("success"):
            self._turn_micropython_recovery_state["soft_reset_success"] = True
        if (
            name in _MICROPYTHON_DEPLOY_TOOLS
            and self._turn_micropython_recovery_state.get("soft_reset_success")
            and not result_obj.get("success", False)
        ):
            self._turn_micropython_recovery_state["post_reset_deploy_failed"] = True
            ...
            while True:
                ...
                for tc in tool_calls_list:
                    ...
                    try:
                        ...
                        blocked_result = self._should_block_redundant_tool_call(func_name, args)
                        if blocked_result is not None:
                            result_obj = blocked_result
                        else:
                        ...
                        self._record_turn_tool_attempt(func_name, args, result_obj)
